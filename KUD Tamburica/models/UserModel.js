...
module.exports = class User {
    ...
    //dohvat korisnika na osnovu id korisnika (tablica authors)
...
dbGetUserByName = async (user_name) => {
    ...
    FROM authors WHERE user_name = '` + user_name + `'`;
    try {
...
dbGetUserByEmail = async (user_email) => {
    const sql = `SELECT id, user_name, first_name, last_name, email, password, role
    FROM authors WHERE email = '` + user_email + `'`;
    try {
        const result = await db.query(sql, []);
        return result.rows;
    } catch (err) {
...
dbNewUser = async (user) => {
    const sql = "INSERT INTO authors (user_name, first_name, last_name, email, password, role) VALUES ('" +
